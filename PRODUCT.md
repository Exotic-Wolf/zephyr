...
- **Brand direction:** warm summer-night breeze, glamorous live-social energy, premium dark/gold identity; the current mascot icon can stay for internal testing but should not be the launch-grade identity.
...
- Update **Canonical Grade Dashboard** whenever the user asks for latest grades or a module's grade changes; keep percentages and grades in one consistent table.
...
- For every uploadable mobile build, bump the store build number/code above the last uploaded value, verify generated platform metadata, and record version, build number/code, command, artifact path, SHA-256, size, date, and caveats here before handoff.
...
| P0 | Done | Codex | Harden Firebase Inbox permission recovery | Inbox now fetches a fresh session-bound Firebase custom token before subscribing, hides raw Firebase permission errors behind a polished retry state, and root background inbox badge listeners fail quietly instead of throwing unhandled stream errors |
...
| P0 | Done | Codex | Generate Android session/inbox AAB `1.0.8+9` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; latest 9 Jun stability rebuild passed direct Gradle `:app:bundleRelease`, manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.8`, version code `9`; SHA-256 `4de5d51fa42b72889a290691fea33048322b9218dc30b69115d0c9d83d10b303` |
| P0 | Done | Codex | Make Android release version source deterministic | Direct Gradle release bundle builds now read `apps/zephyr-mobile/pubspec.yaml` version directly, avoiding stale `android/local.properties` version codes during Play upload retries |
| P0 | Done | Codex | Generate Android Play retry AAB `1.0.9+10` | Google Play rejected version code `9` as already used, so `pubspec.yaml` was bumped to `1.0.9+10`; direct Gradle `./gradlew :app:bundleRelease` passed on 9 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.9`, version code `10`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`, size `70M`, SHA-256 `a1e32a7cd449c4570eab3b40548d8fb2cb14c49735e0818b537f215d78a52860` |
| P0 | Done | Codex | Fix Inbox Firestore permission-denied regression | Root cause: production chat/message docs can preserve legacy extra fields, while receipt/read metadata updates and strict Firestore rules could still throw `cloud_firestore/permission-denied`; mobile receipt updates are now best-effort and Firestore rules allow unchanged legacy fields while still rejecting arbitrary new fields |
| P0 | Done | Codex | Deploy Inbox-compatible Firestore rules | `firebase deploy --only firestore:rules --project zephyr-495115` passed on 9 Jun 2026; production release `cloud.firestore` now points to ruleset `c169f6ce-aaad-4d00-ac33-8911c2203aaa` |
| P0 | Done | Codex | Generate Android Inbox recovery AAB `1.0.10+11` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 9 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.10`, version code `11`; artifact size `70M`, SHA-256 `b7c1c4f6caa761cec3dbafb05a740ce2e48117c110e7ee58c927137a54472eec` |
| P0 | Done | Codex | Fix stale-session takeover UX | When the same account logs in on a newer phone, the older running app now treats stale API/Firebase permission/session errors as global session expiry, cancels root realtime listeners, clears local auth/Firebase state while preserving preferences, returns to sign-in, and shows truthful “signed in elsewhere” copy instead of leaving Inbox reconnecting |
| P0 | Done | Codex | Release active session on logout and clarify takeover copy | Mobile logout now calls backend `POST /v1/auth/logout`; backend revokes exactly the current API session and clears Firestore/RTDB `session_controls` only if they still point to that session. Older phones still must sign in again after takeover, but the notice now says the account **was signed in** elsewhere so it remains truthful after the other device logs out |
| P0 | Done | Codex | Generate Android stale-session UX AAB `1.0.11+12` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 9 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.11`, version code `12`; artifact size `70.4 MB`, SHA-256 `5bcc97b695ffad2b433aacde50d4df7e9875e32328ea5fbb8b34a8838d12fb2e`; build caveats remain known Agora namespace and R8 Kotlin metadata warnings |
| P0 | Done | Codex | Generate Android logout/session AAB `1.0.12+13` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.12`, version code `13`; artifact size `70.4 MiB / 73,848,080 bytes`, SHA-256 `e01ba4d35206d29e23a43d5e165e5bd4d388d1db6ff243aee4ac2f5843e0807f`; build caveats remain known R8 Kotlin metadata warnings and Gradle 9 deprecation warning. Backend `/v1/auth/logout` must be deployed to Render before current-device logout revocation can be fully smoke-tested |
| P0 | Done | Codex | Fix tablet re-login stale session notice | Onboarding now dismisses the previous takeover notice as soon as Google/Apple sign-in starts and clears the parent root-shell notice, so a tablet can sign in again after a stale-session event instead of looking blocked by old copy |
| P0 | Done | Codex | Generate Android tablet-login recovery AAB `1.0.13+14` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.13`, version code `14`; artifact size `70.4 MiB / 73,845,982 bytes`, SHA-256 `c6e68a13a397ffed242917ecffafbf1b7462108052964462914f63db4a84ee40`; build caveats remain known Agora namespace warning, R8 Kotlin metadata warnings, and Gradle 9 deprecation warning. Backend `/v1/auth/logout` must be deployed to Render before current-device logout revocation can be fully smoke-tested |
| P0 | Done | Codex | Fix false another-device lockout on expired local sessions | Mobile restore now shows the “signed in on another device” notice only for the backend's real session-moved case. Normal invalid/expired saved tokens clear silently to regular sign-in, so stale backend/local state cannot falsely trap a user who is not logged in anywhere |
| P0 | Done | Codex | Generate Android false-lockout recovery AAB `1.0.14+15` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.14`, version code `15`; artifact size `70.4 MiB / 73,845,391 bytes`, SHA-256 `b3e32e72aa3bae83a8db5129d45819e1e941838ec13c3b0bf0401312360a0806`; build caveats remain known Agora namespace warning, R8 Kotlin metadata warnings, and Gradle 9 deprecation warning |
| P0 | Done | Codex | Fix tablet re-login realtime false-takeover loop | Root cause: Home opened the incoming-call RTDB listener before Firebase Auth was refreshed with the new session-bound custom token, and generic Firebase `permission-denied` was classified as “another device.” Home now signs into Firebase first, starts RTDB listeners only after active-session claims are ready, and treats Firebase permission failures as recoverable unless a backend `getMe` check proves the API session is actually stale |
| P0 | Done | Codex | Generate Android tablet re-login recovery AAB `1.0.15+16` | Built signed release bundle at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.15`, version code `16`; artifact size `70.4 MiB / 73,846,655 bytes`, SHA-256 `2f31cac6c5151b44bc6b924ef64bd0c209ded7a51dc1d713e985a646540beb55`; build caveats remain known Agora namespace warning, R8 Kotlin metadata warnings, and Gradle 9 deprecation warning |
| P0 | Done | User | Smoke Android tablet re-login recovery on AAB `1.0.15+16` | User confirmed on 10 Jun 2026 that the tablet can sign in successfully and no longer loops on “This account was signed in on another device” after the RTDB permission false-takeover fix |
| P0 | Done | Codex | Upgrade Inbox media composer and photo authorization path | Thread composer now uses a Tango-style plus tray with Camera, Video-soon, and Photos entries; selected photos stage as an attachment draft beside the normal message input; sending creates an immediate optimistic image bubble with caption, upload progress, and in-thread retry using the same idempotency key. Thread entry refreshes the session-bound Firebase custom token before listening/uploading, reconnect retry cancels old listeners, and Firestore rules now require valid `imageUrl` on image messages while rejecting text messages with image URLs |
| P0 | Done | Codex | Generate Android Inbox media AAB `1.0.16+17` | `pubspec.yaml` bumped to `1.0.16+17`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026 in 4m39s; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.16`, version code `17`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.4 MiB / 73,862,115 bytes`, SHA-256 `17708f65c91232dd59d135053a1ebe875c736eb4201b048f1b859e11cf4f945d`; build caveats remain non-fatal R8 Kotlin metadata warnings and Gradle 9 deprecation warning |
| P0 | Done | Codex | Warm Inbox and Thread from local Firestore cache | `FirebaseChatService` now keeps parsed conversation/message snapshots in memory, can read Firestore's local cache before the live listener finishes, Inbox/Thread seed from cached data first, and both screens reuse the already-initialized Firebase session instead of paying a fresh token network hop on normal entry. Retry still forces a token refresh for recovery |
| P0 | Done | Codex | Fix photo+message false-fail and deploy matching Firebase rules | Photo sends now treat FCM push as best-effort after the Firestore commit and verify the optimistic message by idempotency key before showing retry, so an uploaded/committed image cannot be falsely marked failed by a secondary side effect. `firebase deploy --only firestore:rules,storage --project zephyr-495115` passed on 10 Jun 2026; production Firestore now points to ruleset `ce73e28e-6f6a-4846-bcea-19d6ead8e10f`, Storage rules were confirmed current and re-released |
| P0 | Done | Codex | Generate Android photo-message recovery AAB `1.0.17+18` | `pubspec.yaml` bumped to `1.0.17+18`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026 in 1m24s; merged manifest verifies package `com.zephyr.zephyr_mobile`, version name `1.0.17`, version code `18`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.4 MiB / 73,861,156 bytes`, SHA-256 `4936fd95c56794dd353089a4d945b771c27f7bb0f744331c62993a1704f992e9`; build caveats remain non-fatal R8 Kotlin metadata warnings and Gradle 9 deprecation warning |
| P0 | Done | Codex | Generate Android overnight stability AAB `1.0.18+19` | `pubspec.yaml` bumped to `1.0.18+19`; direct Gradle `./gradlew :app:bundleRelease` passed on 10 Jun 2026 in 1m27s; merged and packaged manifests verify package `com.zephyr.zephyr_mobile`, version name `1.0.18`, version code `19`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.4 MiB / 73,863,252 bytes`, SHA-256 `01e497fd6daa4eb99f993dc86909f263a3bc1fa73c5e9eab4112a29d76be3513`; build caveats remain non-fatal Agora duplicate namespace warning, R8 Kotlin metadata warnings, and Gradle 9 deprecation warning |
| P0 | Done | Codex | Generate Android cache-performance AAB `1.0.19+20` | `pubspec.yaml` bumped to `1.0.19+20`; direct Gradle `./gradlew :app:bundleRelease` passed on 11 Jun 2026 in 1m57s; merged and packaged manifests verify package `com.zephyr.zephyr_mobile`, version name `1.0.19`, version code `20`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.4 MiB / 73,870,380 bytes`, SHA-256 `d13836674a8d6b986ebb625b2a856743f2d4a2bf8bd868f7bcaca7a41f0e5e9b`; build caveats remain non-fatal R8 Kotlin metadata warnings and Gradle 9 deprecation warning |
| P0 | Done | Codex | Fix user-initiated logout false takeover notice | Mobile logout now clears the saved token and exits Home before backend/Firebase cleanup finishes, suppresses late session-expired callbacks during the logout window, and has widget coverage proving user logout cannot surface the “signed in on another device” notice |
| P0 | Done | Codex | Generate Android logout-notice recovery AAB `1.0.20+21` | `pubspec.yaml` bumped to `1.0.20+21`; `flutter analyze` passed, `flutter test` passed 14/14, and direct Gradle `./gradlew :app:bundleRelease` passed on 11 Jun 2026 in 1m36s; merged and packaged manifests verify package `com.zephyr.zephyr_mobile`, version name `1.0.20`, version code `21`; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.4 MiB / 73,870,157 bytes`, SHA-256 `a215a55eac634aa3a383d474609e280c079e1a3d2a32bcae5e1784775c535c8e`; build caveats remain non-fatal Agora duplicate namespace warning, R8 Kotlin metadata warnings, and Gradle 9 deprecation warning |
| P0 | Done | Codex | Make push notification delivery active-session scoped | Backend `device_tokens` now stores `session_id`/`device_id`, token registration binds to the authenticated active session, reused app tokens are removed from older accounts, new login removes older-session tokens, logout deletes the current session's tokens, push/read-receipt delivery only selects tokens for the user's current active session, and invalid FCM tokens are pruned after send attempts. Mobile also listens for FCM token rotation and deletes the local FCM token on explicit logout after the backend unregister attempt. `pnpm check`, `flutter analyze`, and `flutter test` passed on 11 Jun 2026; AAB `1.0.21+22` includes the mobile side. Needs Render deploy for device smoke |
| P0 | Done | Codex | Harden chat photo upload and media regression coverage | Chat photos are now prepared before Firebase Storage upload: decode/orientation is handled off the UI thread, images are resized to a 1440px max edge, JPEG-compressed under a 4 MiB app cap so they stay below the 5 MiB Storage rule, saved under `chats/{chatId}/{senderId}/chat_*.jpg`, and uploaded with immutable cache metadata. The thread retries once after refreshing the session-bound Firebase token when Storage/Firestore returns authorization errors, iOS photo/camera permission copy now matches chat media, Android declares image-read permissions, and `flutter test` includes bounded-JPEG and non-image rejection regression tests |
| P0 | Done | Codex | Generate Android chat-media/logout-push AAB `1.0.21+22` | `pubspec.yaml` bumped to `1.0.21+22`; root `pnpm check` passed on 11 Jun 2026 and now includes realtime rules, backend unit/e2e, real Postgres DB race, backend build, Flutter analyze, and Flutter tests 16/16 including chat media preparation. Direct Gradle `./gradlew :app:bundleRelease` passed on 11 Jun 2026 in 1m34s; merged and packaged manifests verify package `com.zephyr.zephyr_mobile`, version name `1.0.21`, version code `22`, and Android image-read permissions; artifact `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`; artifact size `70.8 MiB / 74,251,964 bytes`, SHA-256 `aa9ce83c5b0eb0f06f05d833eeeacbf217c46a2edb40e03094d4b3fe47588911`; build caveats remain non-fatal R8 Kotlin metadata warnings and Gradle 9 deprecation warning. Backend active-session push cleanup still needs Render deploy before no-push-after-logout smoke is meaningful |
...
| P0 | Done | Codex | Wire realtime/backend/mobile gates into CI/default checks | Root `pnpm check` runs RTDB + Firestore + Storage rules, backend unit tests, backend e2e tests, real Postgres DB race tests, backend build, Flutter analyze, and Flutter tests. GitHub Actions runs backend/realtime plus Flutter analyze/test gates on PRs and `main`/`dev` pushes |
...
| P1 | Done | Codex | Refresh stale non-Markdown contracts/test helpers/source comments | OpenAPI now reflects OAuth + session-bound Firebase tokens with no guest-login contract; smoke helpers no longer call guest login and use real bearer tokens for authenticated checks; e2e tests use an internal test-session factory; stale LiveKit/source comments were updated to Agora/Firebase truth |
...
| P2 | Done | Codex | Generate Zephyr Live premium brand concept asset | Stored non-wired A+ direction concept at `apps/zephyr-mobile/assets/brand_concepts/zephyr_live_premium_mark_concept.png`; current mascot icon/splash remain active for internal testing |
...
3. Deploy the backend active-session push/logout changes to Render, upload Android AAB `1.0.21+22`, then smoke: explicit logout returns to plain sign-in with no false another-device warning, no push is received after logout, expired/invalid saved token returns to plain sign-in, OAuth login from any device succeeds, tablet re-login is not bounced by pre-auth RTDB permission-denied, latest login invalidates the older API/Firebase/push sessions, older running device returns to sign-in with the “signed in elsewhere” notice instead of Inbox reconnecting, current-device logout revokes the active API/Firebase/session-bound push controls, Inbox opens without raw Firebase permission text, optimistic text send, failed-send retry, plus-tray photo + message send, bounded photo upload/retry, repeat Inbox/Thread entry paints from warm cache, receipts, block, report, logout/offline.
...
| Area | Current state |
|---|---|
| Overall solution grade | B+ / A- for internal testing today. The architecture and core safety rails are strong. Honest completion estimate: **92% ready for closed/internal testing**, **82% ready for a minimum public Android launch**, and **65% complete against the full Zephyr Live v1 vision**. The remaining gap is broader manual smoke sign-off, store/IAP validation, premium live, gift expansion, brand/store assets, and iOS/admin launch operations. |
| Verified checks | Expanded root `pnpm check` passed on 11 Jun 2026 and now covers RTDB rules, Firestore rules, Storage rules, backend unit tests, backend e2e tests, real Postgres DB race/idempotency tests, backend build, Flutter analyze, and Flutter tests. Backend unit tests include active-session-only push tokens, token removal on logout, reused app-token reassignment between accounts, invalid-token pruning, and old-session token exclusion after latest-login-wins; backend e2e passed 6/6 including `POST /v1/auth/logout` revocation and push-token eligibility removal. Latest mobile pass on 11 Jun 2026: `flutter analyze` passed and `flutter test` passed 16/16 including takeover notice dismissal during re-login, expired-token-vs-moved-session classification, Firebase permission-denied not being treated as takeover by itself, user-initiated logout suppressing false another-device notice, product-safe Firebase/storage/network error copy, bounded chat-image JPEG preparation, and non-image rejection before Storage. Latest Inbox performance pass on 10 Jun 2026: warm in-memory + Firestore local-cache seeding for Inbox and Thread plus skipping redundant Firebase custom-token refresh on normal repeat entry; iPhone 17 Pro Max simulator debug smoke against Render opened For you, Inbox, and a message thread twice with cached list/message paint and no visible spinner/Firebase error. Latest Inbox media/rules pass: `pnpm test:firestore:rules` passed 11/11 including photo+caption transactional send shape, and `pnpm test:storage:rules` passed 4/4 for active-session owner-scoped chat images. Production Firebase rules deploy passed on 10 Jun 2026: Firestore release now points to `ce73e28e-6f6a-4846-bcea-19d6ead8e10f`, Storage was confirmed current and re-released. Direct Gradle `./gradlew :app:bundleRelease` produced signed AAB `1.0.21+22` on 11 Jun 2026 at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`, SHA-256 `aa9ce83c5b0eb0f06f05d833eeeacbf217c46a2edb40e03094d4b3fe47588911`, size `70.8 MiB / 74,251,964 bytes`; the backend active-session push fix still needs Render deploy before no-push-after-logout smoke is meaningful. |
| Known failing check | Local `flutter build appbundle` wrapper reported native debug-symbol stripping failure; direct Gradle `:app:bundleRelease` succeeds, now reads version metadata from `pubspec.yaml`, and produced the uploadable signed AAB. `flutter doctor -v` still reports Android cmdline-tools missing and Android license status unknown on this Mac. Release build still emits known Agora namespace and R8 Kotlin metadata warnings. |
| Auth/session | A+. OAuth login carries a stable app-install device id; the backend maintains one active mobile API session per account, deletes older session rows, mirrors the active session to Firebase `session_controls`, mints Firebase custom tokens with `sessionId`/`deviceId` claims, and binds push tokens to the same active session. Firestore, RTDB, Storage, and push delivery now share the same active-session truth. A running older device now handles confirmed stale-session rejection as a global session-expired event, clears auth/Firebase local state, preserves preferences, and returns to sign-in with truthful “was signed in elsewhere” copy. The sign-in screen clears that stale notice when the user tries to sign in again, expired/invalid local tokens no longer pretend another device is active, generic Firebase permission-denied during fresh login is verified against the backend before any takeover notice is shown, user tablet smoke confirms AAB `1.0.15+16` no longer loops on re-login, AAB `1.0.20+21` suppresses false another-device copy after explicit user logout, and AAB `1.0.21+22` adds local FCM token deletion plus active-session push registration. Current-device logout revokes the API session, clears Firebase session controls only when they still match that session, deletes backend push tokens for that session, and mobile deletes its local FCM token. |
...
| Messaging/inbox | A+ core and media. Firestore/Storage rules, transactional sends, backend-verified best-effort push, active-session-scoped push token selection, block/report ownership, bounded JPEG photo preparation before Storage upload, optimistic text sends, optimistic photo+message sends, committed-message lookup before failed-state display, in-bubble failed-send retry, session-bound Firebase auth refresh on recovery, warm in-memory + Firestore local-cache seeding for repeat Inbox/Thread entry, redundant Firebase-token network hops skipped on normal repeat entry, secure `participantIds` inbox listing with production backfill, best-effort delivery/read receipts, legacy-compatible production rules, product-safe Firebase/storage/network error copy, polished retry state for recoverable permission failures, backend verification before treating Firebase permission-denied as stale session, global stale-session exit for confirmed second-device takeover, production Firestore/Storage rules deployment, and Android AAB `1.0.21+22` are ready for manual smoke. Active-session push cleanup needs Render deploy before the no-push-after-logout smoke is meaningful. |
| Performance/perceived speed | A- moving toward A. Inbox and Thread now avoid cold-looking repeat entrances by seeding from the hot service cache and Firestore's device cache before live listeners finish, normal repeat entry avoids a backend Firebase-token round trip when Home already initialized the active session, and chat photo decode/resize/compression runs off the UI thread before upload. The honest remaining gap is measured real-device profiling: first-ever cold login, feed image decode/cache pressure, low-network traces, frame timings, and release-build startup metrics. |
...
| Brand/logo | Internal-test mascot remains active. Launch direction is now Zephyr Live: warm summer-night breeze, glamorous dark/gold identity. Stored non-wired concept asset: `apps/zephyr-mobile/assets/brand_concepts/zephyr_live_premium_mark_concept.png` SHA-256 `5ecc082a19e6339366c0e38cb19af0d45ee8c1cd05f2d04a1d80a1f8ff4304a9`. |
...
| Documentation status | A+. `PRODUCT.md`, Copilot instructions, OpenAPI, smoke helpers, e2e helpers, and source comments now agree on the current Zephyr Live direction: no product guest login, OAuth-only onboarding, session-bound Firebase auth, Firebase RTDB/Firestore realtime, Agora media, and backend-owned economy/realtime fan-out. |

---

## Canonical Grade Dashboard (11 Jun 2026)

This is the single source for "latest grades." Every future grade request should read from this table, and every meaningful implementation should update the affected rows.

| Area | Completion | Grade | Current blocker / honest read |
|---|---:|---|---|
| Closed/internal testing readiness | **92%** | A- | Strong enough to keep testing now. The default `pnpm check` gate now includes realtime rules, backend unit/e2e, real Postgres DB race, backend build, Flutter analyze, and Flutter tests. Code proves active-session-scoped push tokens, logout push-token removal, invalid token pruning, and bounded chat-image preparation. Android AAB `1.0.21+22` is built with Inbox media hardening, bounded JPEG upload preparation, product-safe Firebase/storage/network error copy, For you feed resiliency fixes, Inbox/Thread warm-cache performance pass, explicit-logout false takeover notice fix, and local FCM token deletion on logout. Backend active-session push cleanup still needs Render deploy. Needs focused two-device smoke and bug capture. |
| Minimum public Android launch | **82%** | B+ | Close, with better automated proof, feed resiliency, media-send hardening, and a fresh AAB, but still blocked by manual two-account call/inbox/live smoke, Play IAP product/purchase smoke, store listing/assets, Render billing stability, and key UX passes. |
| Full Zephyr Live v1 vision | **65%** | C+/B- | Still needs premium live, reusable gifts across inbox/calls/random/premium live, launch-grade brand/icon/splash, deeper Me/revenue controls, admin/moderation tooling, and iOS release flow. |
| Onboarding | **100%** | A+ | Manual OAuth smoke should continue on real devices/accounts, but product architecture is complete. |
| Auth / one-device sessions | **98%** | A+ | Tablet re-login recovery is user-confirmed on AAB `1.0.15+16`; explicit logout false-notice suppression is fixed in AAB `1.0.20+21`; active-session push eligibility and local FCM deletion are included in AAB `1.0.21+22`. Remaining proof: latest-login-wins across any two devices, no push after logout, and current-device logout revocation after backend deploy. |
| RTDB / realtime architecture | **95%** | A+ | Needs manual stress smoke across app foreground/background/call/live transitions. |
| Messaging / Inbox | **98%** | A+ core + media | Plus-tray photo+message UX, optimistic media bubble, committed-message lookup before failed-state display, bounded JPEG photo preparation before Storage upload, in-thread upload retry, active-session-scoped push delivery, recovery-time Firebase token refresh, warm in-memory + Firestore local-cache seeding for repeat Inbox/Thread entry, redundant normal-entry token hops removed, production Firestore/Storage rules deployment, and emulator/backend/mobile proof are in place. Manual smoke still needed for real-device photo send/retry, receipts, block/report, no-push-after-logout after Render deploy, second-device sign-out, and gifts. |
| Performance / perceived speed | **82%** | A- | Inbox/Thread repeat navigation now paints from warm cache while Firebase refreshes, normal repeat entry avoids a backend token round trip, feed boot avoids accessory blocking, and chat photo resize/compression runs off the UI thread before upload. iPhone 17 Pro Max simulator debug smoke confirmed Inbox and Thread opened directly to content twice. A+ requires measured release-device profiling, cold-start optimization, feed image decode/cache tuning, and low-network trace review. |
| Backend economy / wallet ledger | **92%** | A+ | Needs manual paid-flow smoke and production monitoring. |
| Direct + random calls | **88%** | A | Needs two-account smoke: ring, accept, decline, timeout, end, post-call actions, and random host ribbon. |
| Normal live streaming | **86%** | A+ core | Needs manual host/viewer smoke, long-session cleanup, comments/reactions/gifts in production-like conditions. |
| For you / Following feed | **84%** | A | Launch-minimum live-only feed is solid: pull-to-refresh, lazy-load trigger, viewer counts, profile/live tap split, customer Random match empty state, feed boot decoupled from Following, and backend live-room truth wins over stale RTDB presence for ranking. Needs manual iPhone polish pass, live inventory behavior, backend ranking tuning, later live-preview/hide-chrome polish. |
| Me / profile / wallet / settings | **82%** | A | Needs manual profile edit QA, deeper revenue/payout detail, persisted notification controls, and localization. |
| IAP / billing | **72%** | A pending store sign-off | Google Play merchant/product visibility and one real internal-test purchase/refund smoke are still blocking. |
| Gifts overall | **65%** | B+ | Live gifts are strong; reusable gift surface still needs inbox, direct call, random call, and premium live. |
| Brand / store presence | **45%** | B- | Premium concept exists, but app icon/splash/store screenshots/listing are not launch-grade yet. |
| Admin / moderation operations | **35%** | Pending | User-facing block/report exists; back-office moderation panel/workflows are not implemented. |
| Premium live | **20%** | C / not implemented | Paid entry, lock screen, per-minute billing, premium realtime module, and host upgrade flow are still future work. |
| iOS release path | **20%** | Pending | Flutter parity exists, but TestFlight/App Store Connect signing, screenshots, and iOS release smoke are not done. |
| Docs/current-state truth | **100%** | A+ | `PRODUCT.md`, Copilot instructions, OpenAPI, smoke helpers, e2e helpers, and source comments match the current Zephyr Live direction. |

### What Must Be Done Before Public Launch

1. Deploy backend active-session push/logout cleanup to Render, upload Android AAB `1.0.21+22`, then smoke on two devices/accounts: explicit logout returns to plain sign-in with no false another-device notice, no push after logout, expired local token shows plain sign-in, tablet re-login is not bounced by RTDB permission-denied before Firebase custom-token refresh, latest login wins across any two devices, old-device sign-out, current-device logout revocation, Inbox text/photo+message/retry, repeat Inbox/Thread entry from warm cache, direct call, random call, live, profile, logout.
2. Finish Google Play product setup and run one real internal-test purchase/refund.
3. Fix any bugs found from the smoke pass, then rebuild with a fresh version code.
4. Finalize launch icon/splash/store listing/screenshots for Zephyr Live.
...
BASE_URL=https://your-api-domain.com ZEPHYR_SMOKE_ACCESS_TOKEN=<real-app-bearer-token> node scripts/smoke.mjs
BASE_URL=https://your-api-domain.com ZEPHYR_SMOKE_ACCESS_TOKEN=<real-app-bearer-token> ZEPHYR_SMOKE_CREATE_ROOM=true node scripts/smoke.mjs
```
...
| Push notifications (FCM) | ✅ Active-session scoped, needs deploy/AAB smoke | 96% |
...
| ~~Stale non-Markdown contracts/helpers~~ | ✅ Done — OpenAPI, smoke scripts, e2e helper setup, and stale LiveKit/source comments now match current OAuth/Firebase/Agora direction |
...
- [ ] Logout stops push — code fixed with active-session backend filtering + local FCM token delete; verify on deployed backend and next AAB
...
- [x] FCM token cleanup on logout — implemented; real no-push smoke tracked above after backend deploy and next AAB
...
### Messaging / Inbox — 10 Jun 2026 — Overall: A+ core and media, Gift module pending
| Aspect | Grade | Notes |
|--------|-------|-------|
...
| Message correctness | A+ | Text/image sends write message + unread + last-message metadata in one Firestore transaction; idempotency keys use deterministic message IDs so text and media retries cannot double-send. Image+caption uses the same message body path as normal text, not a separate caption object. Images are normalized before upload into bounded JPEG files under a 4 MiB app cap, below the 5 MiB Storage rule. |
| Firestore rules | A+ | Emulator suite proves participant-only chat creation, immutable participants, bounded unread counters, committed-message shape, atomic photo+caption send, image messages requiring an image URL, text messages rejecting image URLs, receiver-only receipts, constrained delete-for-me/delete-for-everyone, and block projection denial. |
| Storage rules | A+ | Chat uploads are owner-scoped and immutable under `chats/{chatId}/{uploaderId}/`; emulator suite proves participant reads, outsider denial, uploader-only writes, image/type/size bounds, and no overwrite/delete. Mobile stores only the binary in Storage; Firestore stores the caption/body, type, and download URL metadata. |
...
| Push | A+ | Client no longer sends arbitrary notification title/body; backend verifies the committed Firestore chat/message before sending FCM and includes a `source=firestore` marker. Push tokens are bound to the authenticated active session, reused app tokens move away from older accounts, logout removes that session's backend tokens, invalid FCM tokens are pruned after send attempts, and mobile deletes the local FCM token on explicit logout. Mobile treats push relay as best-effort after the Firestore commit so push failures cannot make a delivered message look failed. |
| Presence/logout | A+ | Foreground/background availability uses the canonical RTDB cell; logout now exits locally immediately, suppresses false takeover copy, attempts offline write/backend unregister, deletes the local FCM token, revokes the backend API session, and clears session-bound Firebase/push eligibility. |
| UX | A+ core | Inbox/thread include search, live preview, inline translation, optimistic text send, Tango-style plus tray, Camera/Photos selection, image attachment draft, photo+message send, in-bubble upload progress, session-refresh retry for Firebase authorization changes, warm cache paint on repeat entry, committed-message lookup before failed-state display, failed media retry, read receipts, direct-call entry, block, and report. Missing: typing indicator, message reactions, video send, and the reusable gift picker. |
...
| Restore/logout behavior | A+ | An older device clears local backend/Firebase/chat/push state when restore is rejected; if it is already running, confirmed stale API/Firebase session errors now become a root session-expired event that cancels realtime listeners and returns to sign-in with truthful “signed in elsewhere” copy instead of a broken Inbox reconnecting state. Expired/invalid saved tokens no longer show a false another-device warning, and generic Firebase permission-denied during fresh login is treated as recoverable unless backend validation proves the bearer token is stale. Current-device logout revokes the current API session, clears Firebase session controls only if they still point to that same session, removes backend push eligibility, and deletes the local FCM token. |
| Testability | A+ | `pnpm check` passed on 11 Jun 2026 with explicit pre-projection allowance and stale-session denial coverage in RTDB, Firestore, and Storage emulator suites; latest checks add backend unit/e2e coverage for current-session logout revocation, active-session push-token selection, token removal on logout, reused-token reassignment, invalid-token pruning, plus Flutter widget coverage for takeover notice dismissal and expired-token-vs-moved-session classification. |

### Launch Stability Sweep — 11 Jun 2026 — Overall: A code readiness / B+ launch readiness
| Aspect | Grade | Notes |
|--------|-------|-------|
| Automated gates | A+ | Root `pnpm check`, `pnpm test:firestore:rules`, `flutter analyze`, Flutter tests 16/16, backend unit/e2e/build checks, real Postgres DB race tests, and direct Gradle release bundle all pass after stale-session UX hardening, tablet re-login notice fix, expired-token false-lockout fix, RTDB permission false-takeover fix, active-session push-token hardening, and bounded chat-photo upload preparation. Remaining warning noise: Firebase CLI will require Java 21 later, Gradle reports future Gradle 9 deprecations, and R8 prints Kotlin metadata warnings during minify. |
| Inbox resilience | A+ | Inbox now refreshes its session-bound Firebase token before subscribing, uses secure `participantIds` listing, production chat docs were backfilled, root badge listeners handle stream errors, recoverable permission failures show a retry state, and stale-session permission failures trigger global sign-out instead of raw Firebase text or endless reconnecting. |
| User-facing recovery | A | For you and Inbox now have calm retry surfaces for recoverable root load/realtime failures, and second-device takeover has a clear sign-in notice. Remaining raw-error polish exists in secondary flows such as avatar/profile save and the reusable gift tray. |
| Release posture | B+ | Signed AAB `1.0.21+22` builds successfully and verifies version code `22`; production Firestore/Storage rules are deployed for the Inbox media recovery fix. Active-session push-token cleanup is fixed in code and included in the AAB, but still needs Render deployment before no-push-after-logout smoke. Launch is still gated by manual two-account call/inbox smoke, Google Play product purchase smoke, Render paid-tier readiness, premium live, brand/store assets, and remaining launch operations. |
...
| CI/default gate | A+ | Root `pnpm check` now runs RTDB, Firestore, and Storage rules, backend unit tests, backend e2e tests, real Postgres DB race tests, backend build, Flutter analyze, and Flutter tests. GitHub Actions runs backend/realtime plus Flutter analyze/test gates on PRs and `main`/`dev` pushes. |
...
| Test posture | A+ | Backend unit tests/build, backend e2e, Flutter analyze, Flutter tests 16/16, RTDB + Firestore + Storage emulator rules, root `pnpm check`, real Postgres DB race tests, bounded chat-media preparation tests, expanded Flutter widget tests, and GitHub quality gates are in place. Wider mobile widget coverage can continue expanding feature by feature. |
| Documentation accuracy | A+ | `PRODUCT.md`, `.github/copilot-instructions.md`, OpenAPI, smoke scripts, e2e helper setup, and stale source comments now match the current OAuth/Firebase/Agora architecture and Zephyr Live direction. |
| A+ gates | Pending | Finish manual random/direct call smoke, Google Play internal-test IAP smoke, premium live lifecycle, reusable Gift module expansion, and Explore identity polish. |

### Documentation & Current-State Audit — 10 Jun 2026 — Overall: A+
| Aspect | Grade | Notes |
|--------|-------|-------|
| Markdown source of truth | A+ | `PRODUCT.md` carries the current solution snapshot, launch percentages, module completion, TODO tracker, blocker table, endpoint map, architecture direction, and latest audit grades. |
| Copilot instructions | A+ | `.github/copilot-instructions.md` describes the current hard constraints, canonical presence fields, Postgres projection fields, matchmaking gates, RTDB ownership, backend-confirmed live gifts, and build-record rule. |
| Direction governance | A+ | Latest dated snapshots/audits supersede older direction; hard constraints stay hard; product changes now require updating `PRODUCT.md` and Copilot instructions in the same work slice. |
| Historical audit clarity | A+ | Older 3 Jun/5 Jun/7 Jun audit rows remain preserved as history, while the 10 Jun snapshot and audit block state the current truth. |
| Non-Markdown contract truth | A+ | `packages/zephyr-contracts/openapi.yaml` no longer advertises guest login and now includes OAuth device ids plus session-bound Firebase token creation. Smoke scripts no longer call removed guest endpoints; authenticated smoke uses a real bearer token. E2E setup uses an internal test-session factory instead of a product auth route. |
| Source comment truth | A+ | Stale LiveKit/source wording was updated to Agora/Firebase direction; remaining Socket/WebSocket references are explicit prohibition/history/lockfile notes, not active product contracts. |
| Test truth | A+ | Docs record current check results: expanded `pnpm check` with realtime rules, backend unit/e2e, DB race, backend build, Flutter analyze, and Flutter tests 16/16; backend proof for active-session push-token selection, logout token removal, reused-token reassignment, invalid-token pruning, and HTTP logout push eligibility removal; mobile proof for explicit logout false-notice suppression, bounded chat-image JPEG preparation, and non-image rejection; production Firebase rules deployment; iPhone 17 Pro Max simulator Inbox/Thread warm-cache smoke; and direct Gradle AAB `1.0.21+22` build. Test helper naming now reflects internal test sessions instead of user-facing guest login. |
| Next-work clarity | A+ | Immediate work now points only to real launch work: manual For you/call/inbox/Me smoke, Play IAP smoke, gift expansion, premium live, and brand/store assets. |
