...
| P0 | Blocked | User | Manual Google Play internal-test purchase smoke | Fresh local Android AAB `1.0.23+25` exists with onboarding overflow and presence listener recovery fixes; if Play has not already accepted version code `25`, upload it, create/publish `pack_299`, wait for catalog propagation, and retry backend credit/consume/refund smoke |
...
4. Upload/test the fresh Android AAB, then run launch-minimum device smoke. Current local AAB candidate: `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`, version `1.0.23+25`, package `com.zephyr.zephyr_mobile`, SHA-256 `fe59945b9d1c1c1a6d8d379a346c2d4231f9e39eb0dd734c531af9225c1e14d4`, size `74341695` bytes. If Play has already accepted version code `25`, bump the build number before upload.
...
1. Deploy backend active-session push/logout cleanup to Render, upload/test the fresh Android AAB if Play has not already accepted version code `25`, then complete the two-device/account smoke listed in Immediate Next Work.
