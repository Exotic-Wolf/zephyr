...
| P0 | Blocked | Codex/User | Manual Google Play internal-test purchase smoke | Play Console rejected local AAB `1.0.22+24` because version code `24` has already been used; next Android upload must bump `apps/zephyr-mobile/pubspec.yaml` to an unused build number such as `1.0.23+25` or higher, rebuild the AAB, then create/publish `pack_299`, wait for catalog propagation, and retry backend credit/consume/refund smoke |
...
4. Bump Android version/build above used Play version code `24`, rebuild a fresh AAB, upload it to Play/internal testing, then run launch-minimum device smoke. The rejected local AAB remains at `apps/zephyr-mobile/build/app/outputs/bundle/release/app-release.aab`, package `com.zephyr.zephyr_mobile`, version `1.0.22+24`, SHA-256 `3e64f3f95ae9e2669b47d04523a38e486062ca4fe6fc0d509ad0fed97806652f`.
...
1. Deploy backend active-session push/logout cleanup to Render, bump Android build number above used Play code `24`, upload the rebuilt AAB, then complete the two-device/account smoke listed in Immediate Next Work.
