...
| Verified checks | Root `pnpm check` passed on 12 Jun 2026 before the latest Android release bundle. It covers RTDB rules, Firestore rules, Storage rules, backend unit tests, backend e2e tests, real Postgres DB race/idempotency tests, backend build, Flutter analyze, and Flutter tests. Latest mobile tests include takeover notice dismissal, expired-token classification, Firebase permission handling, logout false-notice suppression, product-safe error copy, bounded chat-image preparation, RTDB contract parsing, incoming-call overlay safe-area regression coverage, and non-image rejection before Storage. |
| Known local build caveat | Local `flutter build appbundle` wrapper reported native debug-symbol stripping failure; direct Gradle `:app:bundleRelease` succeeds, reads version metadata from `pubspec.yaml`, and produced the uploadable signed AAB. Latest AAB built on 12 Jun 2026: package `com.zephyr.zephyr_mobile`, version `1.0.22+24`, size `74296151` bytes, SHA-256 `a798294c3cbb721959b5c1ce4095d478932bfc1975dcfd5a6e09a1422b2f46c7`. `flutter doctor -v` still reports Android cmdline-tools missing and Android license status unknown on this Mac. Release build still emits known Agora namespace, R8 Kotlin metadata, and Gradle deprecation warnings. |
