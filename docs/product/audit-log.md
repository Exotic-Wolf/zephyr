...
|---|---|---|---|---|
| Closed/internal testing readiness | A- | Root `pnpm check` passed on 11 Jun 2026; release AAB `1.0.21+22` exists | Manual launch-minimum smoke still open | `current-state.md` |
| Minimum public Android launch | B+ | Core app, backend, Firebase rules, and signed AAB are in place | Two-account smoke, Play IAP product/purchase smoke, store assets, Render billing stability, and launch operations remain | `current-state.md` |
| Full Zephyr Live v1 vision | C+ | Core social/live/call/economy foundation exists | Premium live, reusable gifts, admin/moderation operations, production brand assets, and iOS release remain incomplete | `current-state.md` |
...
| RTDB / realtime architecture | A | Presence/profile/call/live facades, rules, backend projection, and emulator tests are in place | Manual multi-state smoke across foreground/background/call/live remains | `architecture.md` |
| Messaging / Inbox | A | Firestore/Storage rules, transactional sends, backend push relay, bounded image prep, retry, and warm cache are implemented | Real-device media/push smoke, receipts/block/report smoke, and gifts remain | `roadmap-ui.md` |
...
| Gifts overall | B | Live gifts use backend ledger plus Admin RTDB fan-out | Reusable gift module is not wired across inbox, direct call, random call, and premium live | `product-model.md` |
