...
|---|---|---|---|---|
| Closed/internal testing readiness | A- | Root `pnpm check` passed on 11 Jun 2026; release AAB `1.0.21+23` exists | Manual launch-minimum smoke still open | `current-state.md` |
...
| Messaging / Inbox | A- | Text send works; automated Firestore/Storage/media prep gates pass; `pnpm check:firebase:iam` stamps the Storage cross-service IAM role; simulator image upload passes and Firestore has the committed image message with non-empty `imageUrl` | Complete real-device media smoke plus receipts/block/report/no-push-after-logout/gifts smoke before A/A+ | `roadmap-ui.md` |
