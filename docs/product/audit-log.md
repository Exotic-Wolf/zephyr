...
|---|---|---|---|---|
| Closed/internal testing readiness | B+ | Root `pnpm check` passed on 13 Jun 2026; fresh local AAB `1.0.26+27` exists; user-reported smoke on 13 Jun covered Thread image, message send, direct call, Inbox gift, random-call invite path, live path, and one-device takeover | Fix the smoke findings backlog, upload/test `1.0.26+27`, then complete remaining launch-minimum smoke | `current-state.md` |
| Minimum public Android launch | B | Core app, backend, Firebase rules, and signed AAB build path are in place; 13 Jun smoke found concrete launch bugs before public release | Presence disconnect behavior, random-call accept gating, live-host invite visibility, Play upload/acceptance, Play IAP product/purchase smoke, store assets, Render billing stability, and launch operations remain | `current-state.md` |
...
| Onboarding | A- | Google/Apple login, signed-in-elsewhere notice, compact safe-area overflow coverage, legal copy, and profile setup are implemented with tests | Sentry reported a Flutter deactivated-ancestor lifecycle error after iOS OAuth/Firebase auth breadcrumbs; full stack/repro plus launch-minimum smoke remain | `roadmap-ui.md` |
| Auth / one-device sessions | A- | Stable device id, active API session, Firebase session claims, push scoping, and tests are in place; 13 Jun smoke confirmed same-account takeover logs out the older phone | Older device did not show signed-in-elsewhere notice; Sentry OAuth lifecycle stack, Render deploy, and no-push-after-logout smoke remain | `architecture.md` |
| RTDB / realtime architecture | B+ | Presence/profile/call/live facades, fail-closed client contract helpers, presence listener stale-cache invalidation/reattach regression coverage, Flutter module tests, rules, backend projection, and emulator tests are in place | 13 Jun smoke found Wi-Fi-off can leave stale online visible; reconnect/disconnect/onDisconnect behavior needs fix and real-device smoke | `rtdb-contract.md` |
| Messaging / Inbox | B+ | Text/media send path, Admin-written gift cards, reusable gift picker/card UI, once-only unseen gift animation, and automated Firestore/Storage/media prep gates pass; `pnpm check:firebase:iam` stamps the Storage cross-service IAM role; simulator image upload passes and Firestore has the committed image message with non-empty `imageUrl`; user-reported smoke on 13 Jun passed Thread image, message send, and basic Inbox gift | Photo-viewer back focuses keyboard, gift send feels slow, legacy emoji affordances should be removed, and failed-send retry/warm-cache/receipts/block/report/no-push/sign-out/gift retry/re-entry animation smoke remain before A/A+ | `roadmap-ui.md` |
| Performance / perceived speed | B+ | Warm cache, reduced repeat token round trip, nonblocking feed boot, and off-thread chat image prep are in place | 13 Jun smoke found gift sending feels slow; release-device profiling for cold login, gift latency, image cache pressure, low network, frames, and startup remains | `current-state.md` |
...
| Direct + random calls | B | Direct/random flows, shared call screen, receiver ribbon, billing/report paths, and routeability rules exist; basic direct call passed user smoke on 13 Jun | Caller terminal-state gap, random-call caller media/timer before accept, live-host invite visibility, and decline/timeout/next/end smoke remain | `product-model.md` |
| Normal live streaming | B+ | Agora live, RTDB room events, per-viewer audience cells, heartbeat, and backend-trusted live gift fan-out exist; 13 Jun smoke reached live flow | Live chat polish, live gift wiring, live-ended UX review, and production-like host/viewer/comment/reaction/gift/cleanup smoke remain | `product-model.md` |
| For you / Following feed | B+ | Live discovery, host cards, pull refresh, lazy-load trigger, viewer counts, body/profile entry, and Following state exist | Profile placeholder/live-state clarity, iPhone polish, live inventory behavior, ranking tuning, and manual smoke remain | `roadmap-ui.md` |
